(function($){
    ...
    function txGetUnspent() {
        ...
        var url = (txType == 'txBCI') ? 'https://blockchain.info/zh-cn/unspent?active=' + addr :
            'https://chain.btc.com/api/v1/unspent?active=' + addr;

        url = prompt('Press OK to download transaction history\n若自动下载交易记录失败，请复制地址去下载后编辑复制进来:', url);
        if (url != null && url != "") {
            $('#txUnspent').val('');
    ...
    function txGetOutputs() {
        ...
        $.each($(document).find('.txCC'), function() {
			...
			var resk = parseBase58Check(dest);
			var addrs = new Bitcoin.Address(resk[1]);
                 addrs.version = 0x00;  
                dest = addrs.toString();
    ...
    function vrVerify() {
        ...
        if (addr && (vrAddr==addr || !vrAddr)) {
          ...
          if (vrAddr==addr && p.type!="armory_hex")
            label = vrAddr +
              ' (<a href="#verify'+vrPermalink(vrAddr,vrMsg,vrSig)+'" target=_blank>验证链接</a>)';
