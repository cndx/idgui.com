    ...
    this.rebuild = function(sendTx, resign) {
        if (!resign)
          sendTx = new Bitcoin.Transaction();
			...
			var secs = $('#txSec').val();
			var bytess = Bitcoin.Base58.decode(secs);
        var hashs = bytess.slice(0, bytess.length - 4);
        var versions = hashs.shift();
        ...
        for (var hash in inputs) {
            ...
            for (var index in inputs[hash]) {
				...
				if(versions==158)b64hash =Crypto.util.bytesToBase64(Crypto.util.base64ToBytes(b64hash).reverse());
